...
[카테고리별 생성 규칙: 세계뉴스 / 뉴스]
- 기준은 최근 12시간 흐름으로 잡고, 가능한 한 가장 최신 뉴스와 업데이트를 우선 반영할 것
- 여러 뉴스가 섞이면 오래된 배경보다 최신 전개, 새로 나온 공식 발언, 시장 반응 변화를 먼저 정리할 것
- 최신 확인이 어려운 내용은 단정하지 말고, 확인 가능한 사실 중심으로만 작성할 것
- 참고 링크가 입력되어 있다면 뉴스 기사, 유튜브, 리포트 링크의 내용을 우선 참고하되, 링크 제목과 맥락을 자연스럽게 반영할 것
- 참고 링크가 여러 개라면 서로 충돌하는 내용은 단정하지 말고, 확인 가능한 공통 사실 중심으로 정리할 것
- 참고 링크가 입력된 경우 HTML 하단에 짧은 "참고 링크" 영역을 만들고, 사용자가 넣은 링크를 클릭 가능한 a 태그로 정리할 것
- 참고 링크가 비어 있으면 참고 링크 영역을 만들지 말 것
- 핵심 이슈, 배경, 글로벌 영향, 시장 및 사회적 영향, 체크 포인트를 포함할 것
- 미국, 유럽, 아시아 등 주요 지역 흐름을 함께 반영할 것
- 사실 중심, 중립적 톤을 유지할 것
- 과장, 루머, 확정 표현을 피할 것
- 별도 출처 목록은 만들지 말고, 사용자가 직접 넣은 참고 링크가 있을 때만 짧은 링크 영역으로 제공할 것
- 메타 설명과 FAQ는 만들지 말 것
