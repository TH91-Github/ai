...
분량보다 정확성을 우선하며, 사실 확인이 어려운 정보는 과감히 제외합니다.

+ 당신은 단순 정보 작성자가 아니라,
+ 여러 명의 역사 검증자, 문화·기념일 리서처, 스토리 구성자가 함께 검토하며
+ 하나의 콘텐츠를 완성하는 협업형 작성 시스템입니다.
+
+ 다루는 주제는 역사 사건뿐 아니라,
+ 어버이날, 크리스마스, 한글날 같은 기념일·문화적 의미가 있는 날짜도 포함합니다.
+
+ 각각의 담당자는 날짜 정확성, 시대 배경, 문화적 맥락, 대중적 이해도를 교차 검토하며,
+ 서로 일치하는 정보만 최종 결과물에 반영합니다.
+
+ 글은 단순 나열형 설명이 아니라,
+ 사람들이 자연스럽게 빠져들 수 있도록 이야기 흐름으로 흥미롭게 풀어가되,
+ 재미를 위해 확인되지 않은 정보나 과장은 추가하지 마세요.
+
+ 분위기와 몰입감은 살리되,
+ 날짜·인물·장소·사건 흐름 같은 핵심 사실은 반드시 검증된 내용만 사용해 주세요.
...
■ 2단계: 스토리 작성
- 검증된 사실 범위 안에서만 서술해 주세요.
- 검증되지 않은 정보 추가 금지
- 날짜/숫자/인물 재해석 금지
- 검증 범위 밖의 사건, 인물, 수치, 대사, 장면 묘사는 추가하지 마세요.
...
const historicalNarrativeStyleGuide = `
[역사 서술 톤 규칙]
- 역사 강사의 특정 말투를 흉내 내지 말고, 역사 교양서를 읽는 듯한 자연스러운 서술체로 작성해 주세요.
- 문장 끝이 "~했다", "~였다", "~이다"로 반복되지 않게 하고, "~로 이어졌다", "~하게 된다", "~였는데", "~라고 볼 수 있다", "~라는 흐름이었다"처럼 문장 종결을 다양하게 섞어 주세요.
- 문장을 짧게 끊어 서술형 종결인 "~다."를 연속으로 반복하지 말고, 서로 밀접한 내용은 하나의 문장 안에서 자연스럽게 연결해 주세요.
- 인물 소개, 배경 설명, 사건의 진행 과정처럼 같은 흐름에 속하는 정보는 "~였고", "~하면서", "~한 뒤", "~때문에", "~지만" 등의 연결 표현을 활용해 한 문장 또는 한 문단으로 이어 주세요.
- 모든 문장을 억지로 길게 만들 필요는 없지만, 짧은 단문이 세 문장 이상 연속되지 않도록 조절해 주세요. 강조가 필요한 장면이나 핵심 결론에서만 짧은 문장을 사용해 리듬을 살려 주세요.
- 딱딱한 보고서나 교과서 문체가 아니라, 책을 읽듯 자연스럽게 이어지는 흐름으로 써 주세요.
- 설명은 쉽고 선명하게 하되, 과한 감정 연출이나 방송 대본 같은 말투는 피해주세요.
- 사건을 단순히 나열하지 말고, 왜 그런 일이 벌어졌는지 → 그날 어떤 일이 있었는지 → 이후 무엇이 달라졌는지 순서로 자연스럽게 연결해 주세요.
- 문단마다 같은 표현을 반복하지 말고, 한 문단 안에서도 앞뒤 문장이 부드럽게 이어지게 해 주세요.
- 독자가 정보를 읽는 느낌보다, 한 편의 역사 글을 따라 읽는 느낌이 들게 작성해 주세요.
`.trim();
...
다만 위 표현들을 같은 문단 안에서 반복적으로 쓰지는 말고, 필요할 때만 자연스럽게 섞어 주세요.
...
- 구성: 도입부 → 본문 (3~4개 소제목) → 마지막 소제목 이후 2~3개 문단으로 자연스럽게 마무리
...
${historicalNarrativeStyleGuide}${htmlGuide}${imageGuide}${existingBlogGuide}

[추가 지침]
- SEO를 고려하여 키워드를 자연스럽게 배치해 주세요.
- 독자가 실생활에서 바로 적용할 수 있는 실용적인 정보를 포함해 주세요.
- 정보만 나열하지 말고 생활 장면을 자연스럽게 섞어 주세요.
- 예를 들어 음식, 건강, 생활 습관, 재료, 사용법을 다룰 때는 아침 물, 샐러드, 생선 요리, 차로 마시는 방식처럼 실제 생활 맥락이 바로 떠오르게 써 주세요.
- 독자가 읽으면서 "아, 이런 상황에서 쓰이는 이야기구나" 하고 장면이 그려질 수 있게 해 주세요.
...
- 현재 초안처럼 "~인데요 / ~볼 수 있어요 / ~도움이 될 수 있어요" 같은 표현이 반복돼서 AI처럼 느껴지지 않도록, 문장 끝맺음과 리듬을 다양하게 바꿔 주세요.
- 짧은 문장과 조금 더 길게 이어지는 문장을 적절히 섞고, 같은 길이의 문장이 줄줄이 이어지지 않게 해 주세요.
- 문단마다 정보 밀도를 유지하되 군더더기는 줄여 1,200~1,800자 안에서 더 촘촘하게 구성해 주세요.
- 과장된 효능 표현이나 광고처럼 들리는 문장은 빼고, 사실 기반의 부드러운 톤을 유지해 주세요.
- 정보 전달만 하지 말고, 옆에서 차분히 말해주는 블로그 문체로 다듬어 주세요.
- 일반 정보형 글은 역사 글 구조를 적용하지 마세요.
- 다만 역사, 사건, 인물, 날짜, 통계, 제도 변화처럼 사실 흐름이 중요한 주제를 다룰 때는 위 역사 서술 톤 규칙을 적용해 주세요.
- 독자가 글을 읽으며 정보를 얻는 흐름이 중요하며, 보고서·칼럼·역사 서술 구조를 사용하지 마세요.
- 글 마지막의 주의사항이나 참고 문구가 필요하다면 딱딱한 경고문처럼 쓰지 말고, 생활 속에서 참고할 만한 팁처럼 부드럽게 정리해 주세요.
- "정리", "마무리하며", "결론" 같은 소제목을 만들지 마세요.
- 마지막 소제목 이후에는 별도 마무리 소제목 없이 2~3개 문단으로 자연스럽게 글을 닫아 주세요.
- 마지막 부분도 억지로 요약하지 말고, 앞에서 다룬 정보가 생활 속에서 어떻게 이어지는지 편안하게 마무리해 주세요.
- 본문 맨 아래에는 글 내용과 직접 관련된 해시태그를 정확히 10개 추천해 주세요.
- 해시태그는 #생활정보, #건강정보처럼 실제 검색에 쓰기 좋은 형태로 작성하고, 너무 넓거나 본문과 무관한 태그는 피해주세요.
- 해시태그는 한 줄 또는 두 줄로 보기 좋게 정리하고, 관련기사 섹션이 있는 경우 관련기사 섹션 다음에 배치해 주세요.
- 관련 자료가 충분할 경우에만 "📰 관련기사" 섹션을 추가해 주세요.
- 위치는 본문 마지막, 마지막 소제목 이후 마무리 문단 바로 다음입니다.
- 형식은 반드시 <ul> 안에 <li><a href="URL" target="_blank" rel="noopener noreferrer">기사 제목</a></li> 구조를 사용해 주세요.
- 이 주제와 직접 관련된 우리나라 건강/생활정보 뉴스 기사 3개 이상을 선별해 주세요.
- 제목과 링크만 포함하고, 요약이나 설명은 쓰지 마세요.
- 가능한 한 최신 기사 우선으로 골라 주세요.
- 출처는 하이닥, 코메디닷컴, 경향, 조선일보 건강, 헬스조선, 중앙일보 건강, 한국경제 건강처럼 국내 신뢰성 있는 매체만 사용해 주세요.
- 해외 매체, 해외 기관, 영어권 기사, 출처가 불명확한 블로그 링크는 사용하지 마세요.
- 우리나라 관련 기사가 3개 미만이거나 자료가 부족하면 이 섹션은 생략해 주세요.
- 기존 본문 내용은 유지하고, 끝에 관련기사 섹션만 자연스럽게 추가해 주세요.
...
- 최종 HTML은 사용자가 바로 붙여 넣고 .html 파일로 저장할 수 있도록 완결된 구조여야 합니다.
...
export const generateHistoryPrompt = (
  ...
  const koreaGuide = koreaFirst
    ? '대한민국 사건을 우선해서 고르되, 확실한 국내 사건 후보가 3개 미만이면 같은 날짜의 세계사 사건으로 부족한 후보를 보충해 주세요.'
    : '대한민국 사건에만 치우치지 말고 세계사 사건도 함께 고르게 담아 주세요.';

  const toneGuide =
    tone === 'blog'
      ? '딱딱한 해설보다 역사 교양서를 읽는 듯한 자연스러운 서술체로, 쉽고 선명하며 차분하게 풀어 주세요.'
...
- 사건 후보를 보여주는 첫 응답에서는 HTML 생성 금지입니다.
- 첫 응답은 일반 텍스트 목록만 출력해 주세요.
- 사용자가 사건을 선택하기 전까지는 HTML 작성 금지입니다.
...
- 후보 단계에서 확신이 부족한 사건은 후보 목록에 넣지 마세요.
- 대한민국 사건 우선 옵션이 켜져 있더라도, 검증 가능한 국내 사건 후보가 3개 미만이면 같은 날짜의 세계사 사건으로 3~5개 후보를 채워 주세요.
- 같은 날짜의 국내 사건과 세계사 사건을 합쳐도 검증 가능한 후보가 3개 미만일 때만 "정보 없음" 또는 후보 부족을 밝혀 주세요.
...
- 독자가 연표를 읽는 느낌이 아니라 한 편의 역사 글을 따라 읽는 느낌이 들도록 작성하세요.
- 정보를 전달하는 것보다 사건이 어떻게 흘러갔는지를 보여주는 데 집중하세요.
- 문장을 짧게 끊어 서술형 종결인 "~다."를 연속으로 반복하지 말고, 서로 밀접한 내용은 하나의 문장 안에서 자연스럽게 연결해 주세요.
- 인물 소개, 배경 설명, 사건의 진행 과정처럼 같은 흐름에 속하는 정보는 "~였고", "~하면서", "~한 뒤", "~때문에", "~지만" 등의 연결 표현을 활용해 한 문장 또는 한 문단으로 이어 주세요.
- 모든 문장을 억지로 길게 만들 필요는 없지만, 짧은 단문이 세 문장 이상 연속되지 않도록 조절해 주세요. 강조가 필요한 장면이나 핵심 결론에서만 짧은 문장을 사용해 리듬을 살려 주세요.
- 사건 A를 설명한 뒤에는 반드시 "왜?", "그 결과?", "이후 어떤 변화가 있었는가?"를 연결해서 서술하세요.
- 역사 교과서보다 인문 교양서나 차분한 역사 에세이에 가까운 문체를 사용하세요.
- 문장 종결은 "~했다", "~였다", "~이다"로 반복하지 말고 "~로 이어졌다", "~하게 된다", "~였는데", "~라고 볼 수 있다", "~라는 흐름이었다", "~라는 평가를 받는다" 등을 적절히 섞어 사용하세요.
- 지나치게 딱딱한 교과서 문체나 보고서 문체는 피하고, 이해하기 쉬운 역사 이야기 문체로 풀어 주세요.
- 사실을 나열하는 보고서체보다는, 당시 상황을 차례대로 따라가며 읽을 수 있는 블로그형 역사 이야기로 작성해 주세요. 문장 종결과 길이를 다양하게 구성하고, 앞 문장의 정보가 다음 문장으로 자연스럽게 이어지도록 해 주세요.
- 어려운 내용을 풀어줄 때도 쉬운 말로 설명하되, 과장하거나 연기하듯 말하지는 마세요.
- 한 문장 한 문장이 자연스럽게 이어지도록 쓰되, 방송 대본처럼 과한 감탄이나 추임새는 넣지 말아 주세요.
...
- 이미 작성된 HTML 본문을 다듬는 경우에는 내용, 사실관계, 문단 순서, 소제목, 이미지 프롬프트를 그대로 유지하고 문장 종결만 최소 수정해 주세요.
- 이때 "~했다", "~였다", "~이다", "~됐다"가 연속해서 반복되지 않게 조정하고, 짧은 단문이 3개 이상 이어지는 구간은 관련 문장을 자연스럽게 연결해 주세요.
- 수정은 "~로 이어졌다", "~하게 된다", "~였는데", "~라고 볼 수 있다", "~라는 흐름이었다" 같은 종결 표현을 섞는 수준에서만 진행하고, 정보 추가, 삭제, 요약, 재배열은 하지 마세요.
- 역사적 사실, 날짜, 인물, 숫자는 절대 변경하지 말고, HTML 구조와 CSS도 수정하지 마세요.
- 수정 후 전체 본문에서 "~다." 종결이 연속되는 구간이 남아 있는지 한 번 더 점검해 주세요.
...
- 숫자, 규모, 기간 같은 구체 정보는 확인 가능한 경우에만 포함해 주세요.
- 불확실한 수치나 자료마다 다르게 전해지는 숫자는 생략해 주세요.
...
- 실제 기록되지 않은 대사, 독백, 감정 표현은 창작하지 마세요.
- 분위기 연출보다 사건 흐름과 맥락 전달을 우선하세요.
...
- 최종 응답에는 HTML 외의 어떤 텍스트도 출력하지 마세요.
- "다음은 HTML입니다", "작성해드렸습니다", "복사해서 사용하세요" 같은 문구를 출력하지 마세요.
- 코드블록(\`\`\`), writing block(:::)을 사용하지 마세요.
- 반드시 완전한 HTML 문서 형태로만 출력하세요.
...
- 역사 글 마지막 소제목은 "마무리", "정리", "결론", "마무리하며" 같은 일반적인 제목을 사용하지 마세요.
- 마지막 소제목은 사건의 결과, 여운, 변화, 의미를 담은 자연스러운 문장형 제목으로 작성해 주세요.
- 예를 들어 "그날의 외침은 끝나지 않았다", "거리에서 시작된 변화", "바다에 남겨진 교훈", "침몰 이후 달라진 것들", "지금도 이어지는 약속", "한 장의 카네이션이 전하는 마음" 같은 느낌을 참고하되, 그대로 복사하지 말고 사건마다 새롭게 만들어 주세요.
- 마지막 소제목도 반복 패턴 없이 사건에 맞게 새로 만들고, 너무 상투적으로 보이지 않게 해 주세요.
- 마지막 마무리 문장은 별도 소제목 없이, 오늘 이야기를 조용히 정리해 주는 한두 문장으로 마감해 주세요.
- 글의 맨 아래에는 #오늘의역사 형태의 해시태그를 6~10개 정도 한 줄 또는 두 줄로 자연스럽게 정리해 주세요.
- 관련 자료가 충분할 경우에만 "📰 관련기사" 섹션을 추가해 주세요.
- 위치는 본문 마지막, 마지막 소제목 이후 마무리 문단 바로 다음입니다.
- 형식은 반드시 <ul> 안에 <li><a href="URL" target="_blank" rel="noopener noreferrer">기사 제목</a></li> 구조를 사용해 주세요.
- 이 사건과 직접 관련된 우리나라 기사 또는 국내 기관 참고 자료 3개 이상을 선별해 주세요.
- 제목과 링크만 포함하고, 요약이나 설명은 쓰지 마세요.
- 최신 기사보다 공공기관, 박물관, 기념관, 국가기록원, 역사기관 자료를 우선해 주세요.
- 해외 매체, 해외 기관, 영어권 자료, 출처가 불명확한 블로그 링크, 개인 글, 검증되지 않은 매체는 사용하지 마세요.
- 우리나라 기사 또는 국내 기관 참고 자료가 3개 미만이거나 자료가 부족하면 이 섹션은 생략해 주세요.
- 지금부터 위 규칙대로 바로 진행해 주세요.
- 먼저 오늘 날짜 기준 역사 사건 후보 3~5개만 보여 주세요.
- 오늘 날짜가 아닌 다른 날짜는 후보에 올리지 말아주세요.
- 대한민국 사건을 우선하되, 후보가 3개 미만이면 같은 날짜의 세계사 사건으로 보충해 주세요.
- 아직 HTML은 생성하지 말고 사용자의 선택을 기다려 주세요.
- 확신 부족하면 후보 제외 처리하고, 국내와 세계사 후보를 모두 확인해도 3개 미만일 때만 정보 없음 또는 후보 부족을 안내해 주세요.
...
- 결과물은 내가 다른 AI에게 전달할 요청문이라는 점을 유지하되, 사건 선택 전 단계에서는 후보 목록만 먼저 보여 주세요.${excludeGuide}
